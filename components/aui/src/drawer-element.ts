...
export class DrawerElement extends AUIElement {
	...
	#writeProgress(progress: number): void {
		...
		this.style.setProperty("--aui-drawer-progress", String(progress));
		this.style.setProperty("--aui-drawer-offset", `${sign * (1 - progress) * extent}px`);
	}
