...
Requiring description syntax is an intentionally stricter AUI authoring contract than the alertdialog role's normative accessible-name requirement.
...
One close request may close multiple members created in the same native close-watcher group; AUI does not promise one layer per Escape press.
