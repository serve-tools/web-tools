...
This is a web-component contract informed by Base UI 1.7.0 and the donor AUI; it is not JavaScript API parity with either project.
...
- Donor AUI commit `080ad617486945851d0775278d1c8d215bdf75f7`, especially `src/elements/accordion-element.ts` and `src/mixins/accordion-mixin.ts`
...
This package deliberately keeps functional arrow navigation because it is part of AUI's accepted disclosure contract, while preserving every trigger in the ordinary tab sequence.

## Lifecycle and transition boundary

The direct-child and attribute observers and keyboard listener belong to one `AUIElement` connection interval.
