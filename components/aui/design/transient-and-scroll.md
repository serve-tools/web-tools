...
The host exposes `--aui-drawer-progress`, `--aui-drawer-offset`, and `data-dragging`; author CSS owns positioning, dimensions, transforms, backdrop, and animation.
...
`duration` defaults to 5000 milliseconds, a toast may override it with `data-aui-duration`, and zero is persistent.
...
Each rail receives `--aui-scroll-thumb-size` and `--aui-scroll-thumb-offset`.
