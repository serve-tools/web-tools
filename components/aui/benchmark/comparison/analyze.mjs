...
for (const workload of input.protocol.workloads) {
	const paired = input.pairs.map((pair) => {
		...
		const aui = results.aui.observations.map((observation) => observation.durationMs);
		const baseUI = results["base-ui"].observations.map((observation) => observation.durationMs);
		const auiMedianMs = median(aui);
		const baseUIMedianMs = median(baseUI);
		const auiP95Ms = nearestRank(aui, 0.95);
		...
		return {
			...
			baseUIMedianMs,
			baseUIP95Ms,
			medianRatio: baseUIMedianMs / auiMedianMs,
			p95Ratio: baseUIP95Ms / auiP95Ms,
	...
	analysis.results[workload.id] = {
		absoluteRunSummaryMs: {
			aui: {
				median: median(paired.map((pair) => pair.auiMedianMs)),
				p95: median(paired.map((pair) => pair.auiP95Ms)),
			},
			baseUI: {
