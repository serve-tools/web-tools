# Contemporary AUI / pinned Base UI Checkbox comparison

This suite contemporaneously compares the current production `@serve-tools/aui/checkbox` bundle with pinned `@base-ui/react` 1.7.0 and production React and React DOM 19.2.8.
...
Ratios below are Base UI / AUI JavaScript-completion latency, so values above one favor AUI in this fixture.

| Workload           | AUI run median ms | Base UI run median ms | Median ratio and 95% interval | P95 ratio and 95% interval |
| ------------------ | ----------------: | --------------------: | ----------------------------: | -------------------------: |
...
The one-checkbox production AUI application is 26,665 raw minified bytes.
...
The repeated-mount diagnostic showed that this validation history materially perturbed later AUI mounts.
...
Revision 1 stopped after one complete pair when an individual AUI `update-100-1000` observation measured 0.0900 ms against a 0.0050 ms timer quantum, below the fixed 20-quantum precision minimum.
...
Revision 2 then stopped after four complete pairs when an exact-100 isolated AUI block totaled 0.0950 ms, below the same 20-quantum minimum.
...
The analyzer reports geometric Base UI / AUI latency ratios and two-sided 95% Student-t intervals over paired log ratios.
...
AUI uses its public Checkbox subclass and public `checked` property.
...
COMPARISON_ARTIFACT=/path/to/aui-template-migration-2026-09-04/comparison

mkdir -p "$COMPARISON_ARTIFACT"
cp -R "$HISTORICAL_ARTIFACT/closure" "$COMPARISON_ARTIFACT/closure"
```

Build and preflight may run after the current AUI and Signal DOM distributions are final:

```sh
node components/aui/benchmark/comparison/build.mjs \
  --output-dir "$COMPARISON_ARTIFACT"

node components/aui/benchmark/comparison/preflight.mjs \
  --build "$COMPARISON_ARTIFACT/build-metadata.json" \
  --output "$COMPARISON_ARTIFACT/preflight-result.json"
```

Run formal timing only after the primary agent confirms the machine is quiet:

```sh
node components/aui/benchmark/comparison/run.mjs \
  --confirmed-quiet-slot \
  --build "$COMPARISON_ARTIFACT/build-metadata.json" \
  --preflight "$COMPARISON_ARTIFACT/preflight-result.json" \
  --output-dir "$COMPARISON_ARTIFACT/formal-v3"

node components/aui/benchmark/comparison/analyze.mjs \
  --input "$COMPARISON_ARTIFACT/formal-v3/results.json" \
  --output "$COMPARISON_ARTIFACT/analysis.json"
```
