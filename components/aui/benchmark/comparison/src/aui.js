...
const tagName = "bench-aui-checkbox";
...
const cheapMountInvariant = (count) => {
	...
	if (
		...
		throw new Error("AUI retained-reference mount invariant failed");
	}
...
const cheapUpdateInvariant = (targets) => {
	...
	if (
		...
		throw new Error("AUI retained-reference update invariant failed");
	}
...
const validate = () => {
	if (!form) {
		throw new Error("AUI form is not mounted");
	}
	const failures = [];
	...
	if (failures.length > 0) {
		throw new Error(`AUI semantic validation failed: ${failures.slice(0, 12).join(", ")}`);
	}
...
const update = async (targets) => {
	if (controls.length !== 1000) {
		throw new Error("AUI update fixture requires 1,000 mounted controls");
	}
