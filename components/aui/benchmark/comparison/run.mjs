...
function assertMatchedValidation(workloadId, results) {
	for (const phase of ["afterWarmups", "afterRecorded"]) {
		const aui = coreSink(results.aui[phase]);
		const baseUI = coreSink(results["base-ui"][phase]);
		if (JSON.stringify(aui) !== JSON.stringify(baseUI)) {
			throw new Error(`Semantic sink mismatch for ${workloadId}/${phase}: ${JSON.stringify({ aui, baseUI })}`);
		}
	}
}
