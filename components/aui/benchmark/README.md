# AUI Checkbox repeated-mount diagnostic
...
It does not compare AUI with Base UI, establish release readiness, or replace the frozen August 28 comparison.

The fixture bundles the public `@serve-tools/aui/checkbox` implementation from the current production build.
...
The raw JSON records the revision, dirty AUI paths, runtime, browser, hardware, bundle hash, condition order, every mount and teardown duration, and every validation duration.
