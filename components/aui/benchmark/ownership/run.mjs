...
try {
	...
	await context.route("https://aui-ownership-benchmark.invalid/", async (route) => {
		await route.fulfill({
	...
	await page.goto("https://aui-ownership-benchmark.invalid/");
	await page.addScriptTag({ content: bundle });
	if (smoke) {
		await page.evaluate(() => globalThis.__validateAUIOwnershipFixture());
	} else {
		await page.evaluate(() => globalThis.__runAUIOwnershipBenchmarks());
	}
