...
const outputPath = process.argv[3] ?? "/private/tmp/aui-mount-diagnostic.json";
const build = JSON.parse(await readFile("/private/tmp/aui-mount-diagnostic.build.json", "utf8"));
...
const runCondition = async (condition) => {
	...
	try {
		...
		await context.route("https://aui-benchmark.invalid/", async (route) => {
			await route.fulfill({
		...
		await page.goto("https://aui-benchmark.invalid/");
		await page.addScriptTag({ content: bundle });

		const result = await page.evaluate(
			async ({ condition }) => {
				const benchmark = globalThis.__auiMountDiagnostic;
				const warmups = [];
				const observations = [];
				const validations = [];
...
const status = execFileSync("git", ["status", "--short", "--", "components/aui"], { encoding: "utf8" });
const result = {
	build,
