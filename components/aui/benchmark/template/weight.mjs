...
if (!historicalArtifact || !baselineDistribution || !outputDirectory) {
	throw new Error(
		"Usage: node weight.mjs --historical-artifact <directory> --baseline-distribution <directory> --output-directory <directory>",
	);
}
...
const auiAliases = (distribution) => ({
	...sharedAliases,
	"@serve-tools/aui/checkbox": `${distribution}/components/aui/dist/checkbox-element.js`,
...
const sourceInputs = [
	{
		aliases: auiAliases(baselineDistribution),
		input: "aui-weight.js",
		name: "baseline-aui-checkbox-app",
		selector: "weight-aui-checkbox",
	},
	{
		aliases: auiAliases(repoRoot),
		input: "aui-weight.js",
		name: "candidate-aui-checkbox-app",
		selector: "weight-aui-checkbox",
	},
...
const weights = {
	auiCandidateChangeFromBaseline: difference(
		outputs["candidate-aui-checkbox-app"],
		outputs["baseline-aui-checkbox-app"],
	),
	...
	rawBaseUIIncrementTarget: {
		candidateAUIBytes: outputs["candidate-aui-checkbox-app"].bytes,
		deltaBytes: outputs["candidate-aui-checkbox-app"].bytes - targetBytes,
		met: outputs["candidate-aui-checkbox-app"].bytes <= targetBytes,
		targetBytes,
	},
};
