...
const defineField = (): FieldElement => {
	const name = `aui-field-${crypto.randomUUID()}`;
...
describe("FieldElement", () => {
	...
	test("uses an AUI FACE control as the sole label and submission owner", () => {
		const checkboxName = `aui-checkbox-${crypto.randomUUID()}`;
	...
	test("resolves actual Number and OTP inputs without treating wrapper buttons as controls", async () => {
		const numberName = `aui-number-field-${crypto.randomUUID()}`;
		const otpName = `aui-otp-field-${crypto.randomUUID()}`;
