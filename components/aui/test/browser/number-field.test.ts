...
import type { NumberFieldChangeDetail } from "../../src/number-field-element.js";
...
const define = (): NumberFieldElement => {
	const name = `aui-number-field-${crypto.randomUUID()}`;
...
describe("NumberFieldElement", () => {
	...
	test("emits a cancelable proposal only for authored AUI step actions", () => {
	...
	test("honors cancellation and supports step=any with one-unit AUI button steps", () => {
		const { element, increment, input } = create();
		element.step = "any";
