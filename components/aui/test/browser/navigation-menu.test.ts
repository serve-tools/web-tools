...
import { NavigationMenuElement } from "../../src/navigation-menu-element.js";
...
const create = () => {
	const name = `aui-navigation-menu-${crypto.randomUUID()}`;
...
describe("NavigationMenuElement", () => {
	...
	test("preserves an already-open native disclosure during late upgrade", () => {
		const name = `aui-navigation-menu-late-${crypto.randomUUID()}`;
