...
import { SliderElement } from "../../src/slider-element.js";
...
const define = (): SliderElement => {
	const name = `aui-slider-${crypto.randomUUID()}`;
...
describe("SliderElement", () => {
	...
	test("exposes stable ordered percentage variables on the track part", () => {
		...
		expect(track?.style.getPropertyValue("--aui-slider-value-0")).toBe("20%");
		expect(track?.style.getPropertyValue("--aui-slider-value-1")).toBe("80%");
		expect(track?.querySelector('slot[name="thumb"]')).not.toBeNull();

		element.values = [25, 75];
		expect(track?.style.getPropertyValue("--aui-slider-value-0")).toBe("25%");
		expect(track?.style.getPropertyValue("--aui-slider-value-1")).toBe("75%");
	});
	...
	test("uses native orientation and direction on the actual inputs", () => {
		...
		const track = element.shadowRoot?.querySelector<HTMLElement>("[part=track]");
		expect(track?.style.getPropertyValue("--aui-slider-value-0")).toBe("10%");
		expect(track?.style.getPropertyValue("--aui-slider-value-1")).toBe("40%");
	});
	...
	test("refreshes values and CSS snapshots after native setters and form reset without synthetic events", () => {
		...
		const track = element.shadowRoot?.querySelector<HTMLElement>("[part=track]");
		expect(track?.style.getPropertyValue("--aui-slider-value-0")).toBe("35%");
		form.reset();
		expect(lower.valueAsNumber).toBe(20);
		expect(element.values).toEqual([20, 80]);
		expect(track?.style.getPropertyValue("--aui-slider-value-0")).toBe("20%");
