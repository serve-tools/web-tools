...
import { ContextMenuElement } from "../../src/context-menu-element.js";
...
const create = () => {
	const name = `aui-context-menu-${crypto.randomUUID()}`;
...
describe("ContextMenuElement", () => {
	test("opens at pointer coordinates and prevents the browser menu only on success", async () => {
		...
		expect(popup.style.getPropertyValue("--aui-context-menu-x")).toBe("32px");
		expect(popup.style.getPropertyValue("--aui-context-menu-y")).toBe("48px");
	...
	test("preserves initial native open state and reacquires interaction after adoption", async () => {
		const name = `aui-context-menu-late-${crypto.randomUUID()}`;
		...
		expect(trigger.dispatchEvent(event)).toBe(false);
		expect((element as ContextMenuElement).open).toBe(true);
		expect(popup.style.getPropertyValue("--aui-context-menu-x")).toBe("12px");
		(element as ContextMenuElement).hide();
		element.remove();
		expect(trigger.hasAttribute("aria-haspopup")).toBe(false);
