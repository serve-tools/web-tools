...
const create = <T extends BaseElement>(constructor: new () => T): T => {
	const name = `base-test-${crypto.randomUUID()}`;
...
describe("BaseElement", () => {
	...
	test("does not accumulate listeners or bindings over repeated connections", () => {
		...
		const element = create(
			class extends BaseElement {
				...
				protected connect(connection: BaseElement.Connection): () => void {
					this.ownerDocument.addEventListener("base-test-event", () => ++events, {
		...
		for (let index = 0; index < 100; ++index) {
			document.body.append(element);
			document.dispatchEvent(new Event("base-test-event"));
			element.remove();
			document.dispatchEvent(new Event("base-test-event"));
			expect(Signal.subtle.introspectSinks(value)).toHaveLength(0);
		}
	...
	test("reacquires listeners from the adopted document without recreating layout", () => {
		...
		const element = create(
			class extends BaseElement {
				...
				protected connect(connection: BaseElement.Connection): void {
					...
					owner.addEventListener("base-test-adoption", () => received.push(owner), {
						signal: connection.signal,
					});
				}
			},
		...
		document.dispatchEvent(new Event("base-test-adoption"));
		target.dispatchEvent(new Event("base-test-adoption"));
