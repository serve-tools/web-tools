...
const create = <T extends BaseElement>(constructor: new () => T): T => {
	const name = `base-failure-${crypto.randomUUID()}`;
...
test("layout failure retires bindings and preserves authored children", () => {
	...
	expect(() => BaseElement.prototype.connectedCallback.call(element)).toThrow(failure);
	...
	BaseElement.prototype.connectedCallback.call(element);
...
test.each([
	...
	expect(() => BaseElement.prototype.connectedCallback.call(element)).toThrow(
		"Base layout() must return a TemplateResult or finish synchronously",
	);
...
test("connection failure drains resources and a later connection can retry", () => {
	...
	const connections: BaseElement.Connection[] = [];
	let fail = true;
	const element = create(
		class extends BaseElement {
			override connectedCallback(): void {}
	...
	expect(() => BaseElement.prototype.connectedCallback.call(element)).toThrow(AggregateError);
...
test("cleanup finishes before reentrant insertion starts a new connection", async () => {
	...
	const order: string[] = [];
	let reinsert = true;
	let previous: BaseElement.Connection | undefined;
	const element = create(
		class extends BaseElement {
			protected layout(content: DocumentFragment): void {
				text(value)(content);
...
test("connected moves retain bindings and provide the current resource scope", () => {
	...
	let connects = 0;
	let connection: BaseElement.Connection;
	const movedTo: Node[] = [];
	const element = create(
		class extends BaseElement {
			protected layout(content: DocumentFragment): void {
			...
			protected connect(current: BaseElement.Connection): void {
				++connects;
				connection = current;
			}

			protected moved(current: BaseElement.Connection): void {
...
test.each(["explicit adoption", "direct insertion"])(
	...
	async (method) => {
		...
		const element = create(
			class extends BaseElement {
				...
				protected connect(connection: BaseElement.Connection): void {
					...
					owner.addEventListener("base-adopted", () => received.push(owner), { signal: connection.signal });
				}
			},
		);
		document.body.append(element);
		...
		expect(connections[1]!.signal.aborted).toBe(false);
		document.dispatchEvent(new Event("base-adopted"));
		target.dispatchEvent(new target.defaultView!.Event("base-adopted"));
