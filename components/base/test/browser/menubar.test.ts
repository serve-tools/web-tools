...
import { MenubarElement } from "../../src/MenubarElement.js";
...
const create = () => {
	const menuTag = `base-menubar-menu-${crypto.randomUUID()}`;
	const barTag = `base-menubar-${crypto.randomUUID()}`;
...
describe("MenubarElement", () => {
	...
	test("discovers late-upgraded and dynamically replaced direct menus", async () => {
		const barTag = `base-menubar-late-${crypto.randomUUID()}`;
		const menuTag = `base-menu-late-${crypto.randomUUID()}`;
		customElements.define(barTag, class extends MenubarElement {});
		const element = document.createElement(barTag) as MenubarElement;
