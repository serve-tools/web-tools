...
import { DrawerElement } from "../../src/DrawerElement.js";
...
const create = () => {
	const name = `base-drawer-${crypto.randomUUID()}`;
...
describe("DrawerElement", () => {
	...
	test("validates frozen fraction snaps and writes side-aware CSS values", () => {
		...
		expect(element.style.getPropertyValue("--base-drawer-progress")).toBe("0.5");
