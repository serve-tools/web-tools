...
import { DialogElement } from "../../src/DialogElement.js";
...
const defineDialog = (): { element: DialogElement; name: string } => {
	const name = `base-dialog-${crypto.randomUUID()}`;
...
describe("DialogElement", () => {
	...
	test("supports defined-before-parse children and application-controlled late upgrade", () => {
		const definedName = `base-dialog-${crypto.randomUUID()}`;
		...
		const lateName = `base-dialog-${crypto.randomUUID()}`;
