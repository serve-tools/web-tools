...
import { ScrollAreaElement } from "../../src/ScrollAreaElement.js";
...
const create = async (dir = "ltr") => {
	const name = `base-scroll-area-${crypto.randomUUID()}`;
...
describe("ScrollAreaElement", () => {
	...
	test("rebinds replacements, restores presentation attributes, and reacquires after adoption", async () => {
		...
		expect(railY.style.getPropertyValue("--base-scroll-thumb-size")).not.toBe("");
		railX.style.setProperty("--base-scroll-thumb-size", "author");
		element.remove();
		expect(railY.hasAttribute("aria-hidden")).toBe(false);
		expect(railY.style.getPropertyValue("--base-scroll-thumb-size")).toBe("");
		expect(railY.style.getPropertyValue("--base-scroll-thumb-offset")).toBe("");
		expect(railX.style.getPropertyValue("--base-scroll-thumb-size")).toBe("author");
	...
	test("restores authored style values and priorities after forced ownership", async () => {
		...
		railY.style.setProperty("--base-scroll-thumb-size", "7px", "important");
		railY.style.setProperty("--base-scroll-thumb-offset", "3px", "important");
