...
const create = <T extends DisposableElement>(constructor: new () => T): T => {
	const name = `base-disposable-${crypto.randomUUID()}`;
...
test("a reentrant connection drains its old scope before starting the fresh one", () => {
	...
	document.dispatchEvent(new Event("base-disposable-reentrant"));
...
test("old cleanup cannot remove a deduplicated listener from a reentrant connection", () => {
	...
	const element = create(
		class extends DisposableElement {
			static override readonly disposables: readonly DisposableElement.DisposableInitiator[] = [
				(host) => {
					++connections;
					host.ownerDocument.addEventListener("base-disposable-stable-listener", listener);

					return () => host.ownerDocument.removeEventListener("base-disposable-stable-listener", listener);
				},
				(host) => () => {
					if (connections === 1) {
						document.body.append(host);
	...
	document.body.append(element);
	element.remove();
	document.dispatchEvent(new Event("base-disposable-stable-listener"));
	expect(element.isConnected).toBe(true);
	expect(connections).toBe(2);
	expect(events).toBe(1);
...
test("direct cross-document insertion recreates document-owned resources", () => {
		...
		class extends DisposableElement {
			static override readonly disposables: readonly DisposableElement.DisposableInitiator[] = [
				(host) => {
					...
					owner.addEventListener("base-disposable-adoption", () => received.push(owner), {
						signal: host.disconnectedSignal(),
					});
				},
			];
	...
	document.dispatchEvent(new Event("base-disposable-adoption"));
	target.dispatchEvent(new Event("base-disposable-adoption"));
