...
import type { CheckboxChangeDetail } from "../../src/CheckboxElement.js";
...
const defineCheckbox = (): { element: CheckboxElement; name: string } => {
	const name = `base-checkbox-${crypto.randomUUID()}`;
...
describe("CheckboxElement", () => {
	...
	test("preserves authored tabindex through fieldset and group disabling", () => {
		...
		const groupDefinitions = {
			checkbox: `base-checkbox-${crypto.randomUUID()}`,
			group: `base-checkbox-group-${crypto.randomUUID()}`,
		};
