...
const define = () => {
	const option = `base-option-${crypto.randomUUID()}`;
	const autocomplete = `base-autocomplete-${crypto.randomUUID()}`;
	const combobox = `base-combobox-${crypto.randomUUID()}`;
	const select = `base-select-${crypto.randomUUID()}`;
...
describe("selection elements", () => {
	...
	test("participates as a Field control and preserves custom validity over required-state changes and reset", () => {
		const names = define();
		const fieldName = `base-field-${crypto.randomUUID()}`;
	...
	test("restores an owned popup attribute after detach and recovers values assigned before custom-element upgrade", async () => {
		const optionName = `base-option-${crypto.randomUUID()}`;
		const hostName = `base-combobox-${crypto.randomUUID()}`;
