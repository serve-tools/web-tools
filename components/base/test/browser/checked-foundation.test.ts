...
import { CheckboxGroupElement } from "../../src/CheckboxGroupElement.js";
import { FormAssociatedElement } from "../../src/FormAssociatedElement.js";
...
describe("checked-control foundation", () => {
	test("refreshes Checkbox validation when group disabledness changes behind direct disabledness", () => {
		const checkboxName = `base-checked-foundation-${crypto.randomUUID()}`;
		const groupName = `base-checked-foundation-group-${crypto.randomUUID()}`;
	...
	test("does not dispatch initialization through a consumer override", () => {
		...
		for (const Base of [ConsumerCheckbox, ConsumerSwitch]) {
			const name = `base-checked-foundation-${crypto.randomUUID()}`;
