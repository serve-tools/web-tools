...
const create = <T extends BaseElement>(constructor: new () => T): T => {
	const name = `base-template-conformance-${crypto.randomUUID()}`;
...
describe("production template conformance", () => {
	...
	test.each(renderers)("$name follows Base layout suspension without rebuilding nodes", async (renderer) => {
		const value = new Signal.State("before");
		const clicks = vi.fn();
	...
	test("matches current inert custom-element cloning and upgrade timing", () => {
		const name = `base-template-upgrade-${crypto.randomUUID()}`;
		let constructions = 0;
		let connections = 0;
