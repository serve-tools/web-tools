...
import { clearRealmTimeout, isHTMLElement, setRealmTimeout } from "./_composite.js";
...
export class ContextMenuElement extends MenuElement {
	...
	showAt(x: number, y: number, source?: HTMLElement): void {
		...
		popup.style.setProperty("--base-context-menu-x", `${x}px`);
		popup.style.setProperty("--base-context-menu-y", `${y}px`);
		super.show(source ?? this.trigger ?? undefined);
	}
