...
The opt-in `@serve-tools/base-components/template` entrypoint re-exports Signal DOM's tagged-template API: inert `html` descriptions, `createFragment()`, `TemplateResult`, `TemplateFragment`, `TemplateDirective`, and `PersistentFragment`.
...
```ts
import { Signal } from "@serve-tools/signal";
import { createFragment, html } from "@serve-tools/base-components/template";
...
See [Own a persistent template](../../.agents/skills/serve-tools-base/references/own-a-persistent-template.md) for cleanup, parser, and movement boundaries.
...
They intentionally have no Base replacement class or import.
...
A radio group is defined by its shared nonempty name, form owner, and tree, not by an Base wrapper.
...
Register `SwitchElement` from `@serve-tools/base-components/switch` and style its `control` part and optional `thumb` slot.
...
Checkable items remain non-form-associated buttons: do not add a submission `name`, and Base creates no hidden form value.
...
<app-context-menu>
	...
	<div
		...
		popover
		role="menu"
		style="position: fixed; left: var(--base-context-menu-x); top: var(--base-context-menu-y)"
	>
		<button type="button" role="menuitem">Rename</button>
...
Visually overlapping multiple native tracks depends on engine-specific styling and hit testing, so Base does not claim a portable shared multi-thumb track or noninteractive replacement thumbs.
...
Opening can be canceled; native closing cannot, and Base does not attempt to reopen a canceled close.
...
The current `snapPoint` and CSS properties `--base-drawer-progress` and `--base-drawer-offset` let author styles render the position.
...
The region duration defaults to 5000 milliseconds; `data-base-duration` supplies a per-toast default and `show()` options take precedence.
...
Context and drag/drop use the existing `@serve-tools/client-context` and `@serve-tools/client-input` packages rather than duplicate Base implementations.
The gallery's [integration examples](examples/integrations.ts) connect providers and consumers, refresh a consumer after a move, and give a drop observer the Base connection's abort signal.
Their application element names are examples, not additional exports from `@serve-tools/base-components`.
...
Calendar and File are separate Base components described above; an inline calendar is not the same component as a native date input.
...
```ts
protected connect(connection: BaseElement.Connection) {
...
Base targets browser documents with native dialog/popover, ElementInternals, custom CSS states, CSS positioning, form association, and `DisposableStack`.
For an environment without explicit resource management, install `@serve-tools/polyfill-resource-management` before importing and using Base; Base does not mutate globals on import.
...
This capability keeps tooltip dismissal in the browser's native close-request system; Base does not provide a fallback overlay stack.
...
```shell
npm run typecheck --workspace @serve-tools/base-components
npm run test:browser --workspace @serve-tools/base-components
```
...
```shell
npm run dev --workspace @serve-tools/base-components
```
...
Repository-only consumer guidance lives in [the Base Skill](../../.agents/skills/serve-tools-base/SKILL.md).
It is not packaged or included in the public Skill catalog while Base is private.
