...
for (const condition of ["base", "base-ui"]) {
...
for (const name of [
	...
	result.checks[`matched:${name}`] =
		JSON.stringify(coreSink(result.conditions.base.validations[name])) ===
		JSON.stringify(coreSink(result.conditions["base-ui"].validations[name]));
}

for (const name of ["base-checkbox-app", "base-ui-checkbox-app", "react-native-checkbox-app"]) {
	const browser = await chromium.launch({ headless: true });
	try {
		const errors = [];
