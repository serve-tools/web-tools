...
```html
<base-number-field min="0" max="12" step="1">
...
```html
<base-slider min="0" max="100" step="1">
...
The track part exposes `--base-slider-min`, `--base-slider-max`, and ordered percentage values `--base-slider-value-0`, `--base-slider-value-1`, and so on.
...
```css
base-slider::part(track) {
  background:
    linear-gradient(
      to right,
      transparent 0 var(--base-slider-value-0),
      AccentColor var(--base-slider-value-0) var(--base-slider-value-1),
      transparent var(--base-slider-value-1) 100%
    ) center / 100% 0.25rem no-repeat;
}

base-slider:dir(rtl)::part(track) {
  background-image:
    linear-gradient(
      to left,
      transparent 0 var(--base-slider-value-0),
