...
The table reports medians of process medians and paired geometric Base UI/Base ratios with two-sided 95% Student-t intervals over eight independent log ratios.
Ratios above one favor Base.
...
| Workload                                |  Base ms | Base UI ms | Median ratio, 95% interval | p95 ratio, 95% interval |
| --------------------------------------- | -------: | ---------: | -------------------------- | ----------------------- |
...
Ten independent counterbalanced pairs compared the final public Base Checkbox with Base UI 1.7.0 and production React/React DOM 19.2.8 in fresh Chromium 151 processes.
...
The table reports the median of ten condition-run medians, with Base UI/Base paired geometric ratios and two-sided Student-t 95% intervals over the paired log ratios.
...
| Workload                              |  Base ms | Base UI ms | Base UI/Base | Paired 95% interval | Result                           |
| ------------------------------------- | -------: | ---------: | -----------: | ------------------: | -------------------------------- |
...
Both observed mount medians were slower for Base, but the later validation-history diagnostic below shows that the repeated-mount harness itself needs correction before those absolute values guide runtime optimization.
...
The original ten-pair experiment recorded a material Mount 1,000 regression: Base 205.9500 ms, Base UI 136.1750 ms, ratio 0.743 with a 0.615–0.899 interval.
Its isolated update reached the original timer floor and was inconclusive.
The original Base fixture reused its connected form while React created a new form subtree; the later experiment matched that ownership and attachment boundary.
...
An individual-timing follow-up then passed its precision preflight but stopped during its first formal Base condition: eight of 50 durations were zero, exceeding the fixed 5% limit.
...
| ----------------------------- | -----------------: | ---------: | -----------: |
| Base standalone               |             17,031 |      5,700 |        5,152 |
...
The standalone Base application is smaller, but its 17,031 bytes do not establish an incremental advantage when React is already present.
...
The earlier primary, diagnostic, and individual-precision artifacts remain at `base-comparison-2026-08-28`, `base-checkbox-mount-diagnostic-2026-08-28`, and `base-comparison-followup-2026-08-28` under the same outputs directory.
...
The [durable Base-only diagnostic](../benchmark/RESULTS.md) tested whether running a derived version of the full post-mount validation sink after every sample perturbs the next mount.
...
Both conditions timed the same public production Base operation, kept teardown and validation outside the mount clock, and ended with identical exact form, label, identity, light-DOM, shadow-root, and shadow-element sinks.
...
This does not identify garbage collection or another browser subsystem as the cause, compare current Base with Base UI, or satisfy the release gate.
...
The final measurements cover JavaScript work in Chromium 151 on an Apple M5 Max, not layout, paint, native interaction, scoped Base lifecycles, or heap retention.
...
Also compare the changed Signal DOM and effect machinery with the unchanged web-tools revision `b24edab960932b3dbd1683b691e46555f615ff20` so an Base improvement cannot hide a regression for existing consumers.
...
Do not force only one implementation into synchronous rendering or compare development React with production Base.
