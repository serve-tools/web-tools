...
Right-click records `clientX` and `clientY` once in `--base-context-menu-x` and `--base-context-menu-y` and associates the context area as the native source.
The browser context menu is prevented only when the Base popup actually opens, so canceling native opening preserves the browser default.
