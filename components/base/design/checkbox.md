...
Base keeps one labelable, focusable, form-associated custom-element host.
...
All three tested browser engines exclude both a read-only native checkbox and a `readonly` form-associated custom element from constraint validation; Base preserves this platform behavior.
