...
Give each Base element an explicit binding scope and a separate connection resource scope.
...
Nested Base elements create their own scope.
...
Compare native control and Base event/state behavior where the component delegates to native APIs.

Disconnect work must be proportional to owned bindings and resources, with no DOM traversal.
Disconnected source updates must cause zero Base binding work.
