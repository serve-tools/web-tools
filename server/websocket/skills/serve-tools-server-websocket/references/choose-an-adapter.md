...
- Use `@serve-tools/server-websocket/scope/node` with the optional `ws` peer for `node:http` upgrade events.
- Use `@serve-tools/server-websocket/scope/bun` for `Bun.serve()` upgrade and WebSocket callbacks.
