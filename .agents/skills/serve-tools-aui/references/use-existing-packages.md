...
Do not import an AUI Context, Drag, Drop, or Time class; the package does not duplicate these facilities.
...
Register its `disconnect()` cleanup with the AUI connection before calling `connect()`.
