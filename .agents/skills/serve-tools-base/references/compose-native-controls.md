...
Use native `<button>`, `<input>`, `<fieldset>`, `<form>`, and `<input type="radio">` directly for those families; Base intentionally exports no replacement classes for them.
...
For Checkbox, register `CheckboxElement` from `@serve-tools/base-components/checkbox` under an application tag name.
...
For Switch, import `SwitchElement` from `@serve-tools/base-components/switch` and use the same checked/defaultChecked, form, optional unchecked-value, and event contract as Checkbox.
Switch has no indeterminate state; its optional decorative slot is `thumb` and its presentational part is `control`.

For Checkbox Group, register `CheckboxGroupElement` from `@serve-tools/base-components/checkbox-group` and supply direct Checkbox children with explicit unique values.
