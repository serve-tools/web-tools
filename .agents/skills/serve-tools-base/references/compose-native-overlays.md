...
Register the required class under application-owned names: `PopoverElement` from `@serve-tools/base-components/popover`, `TooltipElement` from `@serve-tools/base-components/tooltip`, `PreviewCardElement` from `@serve-tools/base-components/preview-card`, or `AlertDialogElement` from `@serve-tools/base-components/alert-dialog`.
