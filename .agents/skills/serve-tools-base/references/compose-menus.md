...
Do not put a submission `name` on checkable menu buttons or mirror them with hidden inputs unless the product separately requires form state; Base adds no form value.
...
Right-click writes one viewport point to `--base-context-menu-x` and `--base-context-menu-y` and requests native opening.
...
The browser context menu is prevented only after the Base popup opens, so a canceled opening preserves the browser default.
...
Base releases temporary roles, ARIA, `tabindex`, button types, and popover modes when the selected node changes or the owner disconnects.
