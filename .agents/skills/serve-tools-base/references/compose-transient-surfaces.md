...
Only the handle begins Base pointer gestures.
...
Render from `--base-drawer-progress` and `--base-drawer-offset`.
...
The default duration is 5000 milliseconds; use the region's numeric `duration`, `data-base-duration` on one toast, or a `show()` option to override it.
