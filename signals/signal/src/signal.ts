...
interface Node {
	[Slot.sinks]?: AnyConsumerNode[];
	[Slot.unwatched]?: (() => void) | undefined;
	[Slot.watched]?: (() => void) | undefined;
	[Slot.wrapper]: any;
}

interface SignalNode<TValue = unknown> extends Node {
	[Slot.eq]?: ((this: any, a: any, b: any) => boolean) | undefined;
	[Slot.value]: TValue;
	[Slot.version]: number;
}
...
interface ConsumerNode extends Node {
	[Slot.dirty]: boolean;
	[Slot.sources]: AnySignalNode[];
	[Slot.nextSource]?: number;
}

interface ComputedNode<TValue = unknown> extends SignalNode<any>, ConsumerNode {
	[Slot.callback]: (this: any) => any;
	[Slot.epoch]?: number;
	[Slot.error]?: unknown;
	[Slot.sourceVersions]: number[];
	[Slot.value]: any;
	[Slot.wrapper]: Computed<TValue>;
}

interface WatcherNode extends ConsumerNode {
	[Slot.notify]: (this: Watcher) => void;
	[Slot.wrapper]: Watcher;
}
...
const runHook = (callback: (() => void) | undefined, node: AnySignalNode) => {
	if (callback) {
		...
		try {
			callback.call(node[Slot.wrapper]);
		} finally {
			notifying = false;
		}
	}
...
const addSink = (source: AnySignalNode, sink: AnyConsumerNode) => {
	if ((source[Slot.sinks] ||= []).push(sink) === 1) {
		const computed = source as ComputedNode;

		if (computed[Slot.callback]) {
			for (const dependency of computed[Slot.sources]) {
				addSink(dependency, computed);
			}
		}

		runHook(source[Slot.watched], source);
	}
};

const removeSink = (source: AnySignalNode, sink: AnyConsumerNode) => {
	const sinks = source[Slot.sinks]!;
	...
	if (!sinks.length) {
		...
		if (computed[Slot.callback]) {
			for (const dependency of computed[Slot.sources]) {
				removeSink(dependency, computed);
			}
		}

		runHook(source[Slot.unwatched], source);
	}
};
...
const markDirty = (node: AnyConsumerNode) => {
	...
	if (watcher[Slot.notify]) {
		try {
			watcher[Slot.notify].call(watcher[Slot.wrapper]);
		} catch (error) {
			(notificationErrors ||= []).push(error);
		}
	} else {
		for (const sink of node[Slot.sinks] || []) {
			markDirty(sink);
		}
	}
};
...
const track = (source: AnySignalNode) => {
	...
	const sourceIndex = computing[Slot.nextSource]!++;
	const previousSource = computing[Slot.sources][sourceIndex];

	if (previousSource !== source) {
		if (previousSource && computing[Slot.sinks]?.length) {
			removeSink(previousSource, computing);
		}

		computing[Slot.sources][sourceIndex] = source;

		if (computing[Slot.sinks]?.length) {
			addSink(source, computing);
		}
	}

	computing[Slot.sourceVersions][sourceIndex] = source[Slot.version];
};

const finishTracking = (node: ComputedNode) => {
	while (node[Slot.sources].length > node[Slot.nextSource]!) {
		const source = node[Slot.sources].at(-1)!;

		if (node[Slot.sinks]?.length) {
			removeSink(source, node);
		}

		node[Slot.sources].pop();
		node[Slot.sourceVersions].pop();
	}
};

const sourcesChanged = (node: ComputedNode) => {
	for (let index = 0; index < node[Slot.sources].length; ++index) {
		const source = node[Slot.sources][index];

		if (source[Slot.version] !== node[Slot.sourceVersions][index]) {
			return true;
		}
...
const recompute = <T>(node: ComputedNode<T>) => {
	const previousValue = node[Slot.value];

	node[Slot.value] = signalNode;
	node[Slot.nextSource] = 0;
	...
	try {
		value = node[Slot.callback].call(node[Slot.wrapper]);
		equal =
			previousValue !== err &&
			previousValue !== runHook &&
			(node[Slot.eq]
				? node[Slot.eq].call(node[Slot.wrapper], previousValue, value)
	...
	node[Slot.epoch] = epoch;

	if (error !== err) {
		node[Slot.error] = error;
		node[Slot.value] = runHook;

		++node[Slot.version];
	} else {
		node[Slot.value] = equal ? previousValue : value;
...
const updateComputed = (node: ComputedNode) => {
	if (node[Slot.value] === signalNode) {
		err("no cycle");
	}

	if (!node[Slot.dirty] && node[Slot.epoch] === epoch) {
		return;
	}

	if (node[Slot.value] !== err && !sourcesChanged(node)) {
		node[Slot.dirty] = false;
		node[Slot.epoch] = epoch;
...
class State<T> {
	...
	constructor(value: T, options?: Options<State<T>>) {
		...
		if (options) {
			node[Slot.eq] = options.equals;
			node[Slot.unwatched] = options[unwatched];
			node[Slot.watched] = options[watched];
		}
	...
	set(value: T) {
		...
		if (!(node[Slot.eq] ? node[Slot.eq].call(this, node[Slot.value], value) : Object.is(node[Slot.value], value))) {
			node[Slot.value] = value;
...
class Computed<T> {
	...
	get(): T {
		...
		if (node[Slot.value] === runHook) {
			throw node[Slot.error];
		}

		return node[Slot.value] as T;
	}
}
...
class Watcher {
	...
	watch(...signals: AnySignal[]) {
		...
		for (const signal of signals) {
			...
			if (!node[Slot.sources].includes(source)) {
				node[Slot.sources].push(source);
	...
	unwatch(...signals: AnySignal[]) {
		...
		for (const signal of signals) {
			const source = signalNode(signal);
			const sourceIndex = node[Slot.sources].indexOf(source);

			if (sourceIndex >= 0) {
				removeSink(source, node);

				node[Slot.sources].splice(sourceIndex, 1);
			}
		}
	}
	...
	getPending() {
		...
		for (const source of this[NODE][Slot.sources]) {
			if ((source as ComputedNode)[Slot.callback] && (source as ComputedNode)[Slot.dirty]) {
				pending.push(source[Slot.wrapper]);
			}
		}
