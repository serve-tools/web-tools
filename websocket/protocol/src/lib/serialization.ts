...
export function serialize(root: unknown): ArrayBuffer {
	...
	const visit = (value: unknown): Ref => {
		if (value === undefined) {
			return SentinelRef.Undefined;
		}

		if (typeof value === "number") {
			if (Number.isNaN(value)) {
				return SentinelRef.NaN;
			}
			if (value === Infinity) {
				return SentinelRef.PositiveInfinity;
			}
			if (value === -Infinity) {
				return SentinelRef.NegativeInfinity;
			}
			if (Object.is(value, -0)) {
				return SentinelRef.NegativeZero;
			}
		}
	...
	const encode = (value: unknown): unknown => {
		...
		if (Array.isArray(value)) {
			...
			for (let index = 0; index < value.length; ++index) {
				refs[index] = index in value ? visit(value[index]) : SentinelRef.Hole;
			}
		...
		if (ArrayBuffer.isView(value)) {
			...
			try {
				...
				return [Tag.DataView, visit(view.buffer), view.byteOffset, view.byteLength];
			} catch {}
		...
		if (Object.getPrototypeOf(object) === objectPrototype) {
		...
		switch (tag) {
			case "Date": {
				try {
					...
					return [Tag.Date, Number.isNaN(time) ? "x" : time];
				} catch {
					return record(object);
				}
			}
			case "RegExp": {
				try {
					return [Tag.RegExp, regexpSource.call(value), (value as RegExp).flags];
				} catch {
					return record(object);
				}
			}
			case "ArrayBuffer": {
				try {
					...
					if (arrayBufferDetached?.call(value)) {
						return die();
					}
					...
					return arrayBufferResizable?.call(value)
						? [Tag.ArrayBuffer, offset, byteLength, arrayBufferMaxByteLength!.call(value)]
						: [Tag.ArrayBuffer, offset, byteLength];
				} catch (error) {
			...
			case "Map": {
				...
				const entry: unknown[] = [Tag.Map];

				for (const [key, item] of entries) {
					entry.push(visit(key), visit(item));
				}
			...
			case "Set": {
				...
				const entry: unknown[] = [Tag.Set];

				for (const item of values) {
					entry.push(visit(item));
				}
			...
			case "Boolean":
				try {
					return [Tag.Wrapper, visit(Boolean.prototype.valueOf.call(value))];
				} catch {
					return record(object);
				}
			case "Number":
				try {
					return [Tag.Wrapper, visit(Number.prototype.valueOf.call(value))];
				} catch {
					return record(object);
				}
			case "String":
				try {
					return [Tag.Wrapper, visit(String.prototype.valueOf.call(value))];
				} catch {
					return record(object);
				}
			case "BigInt":
				try {
					return [Tag.Wrapper, visit(BigInt.prototype.valueOf.call(value))];
				} catch {
					return record(object);
				}
			case "Error": {
				...
				const entry: unknown[] = aggregate
					? [Tag.AggregateError, error.message, visit(error.errors)]
					: [Tag.Error, Math.max(0, errorNames.indexOf(error.name)), error.message];

				if (Object.hasOwn(error, "cause")) {
					entry.push(visit(error.cause));
				}
...
export function deserialize(payload: ArrayBuffer | ArrayBufferView): unknown {
	try {
		...
		if (!Array.isArray(metadata) || metadata.length !== 3 || metadata[0] !== protocol) {
			return die();
		}
		...
		const deref = (ref: unknown): unknown => {
			...
			if (ref < 0) {
				return ref === SentinelRef.Undefined || (ref <= SentinelRef.NaN && ref >= SentinelRef.NegativeZero)
					? sentinels[-ref]
					: die();
			}
		...
		const tagged = (index: number, entry: unknown[]): unknown => {
			...
			switch (tag) {
				...
				case Tag.Date:
					if (entry.length !== 2 || (entry[1] !== "x" && typeof entry[1] !== "number")) {
						return die();
					}
				...
				case Tag.DataView:
					if (entry.length !== 4 || !isSize(entry[2]) || !isSize(entry[3])) {
						return die();
					}
				...
				case Tag.Set: {
					...
					for (let item = 1; item < entry.length; ++item) {
						set.add(deref(entry[item]));
					}
				...
				case Tag.AggregateError: {
					if ((entry.length !== 3 && entry.length !== 4) || typeof entry[1] !== "string") {
						return die();
					}
					...
					if (entry.length === 4) {
						define(error, "cause", deref(entry[3]));
					}
		...
		const hydrate = (index: number): unknown => {
			...
			if (entry === null || typeof entry !== "object") {
				return (output[index] = entry);
			}

			if (Array.isArray(entry)) {
				if (typeof entry[0] === "string") {
					return tagged(index, entry);
				}
				...
				for (let item = 0; item < entry.length; ++item) {
					if (entry[item] !== SentinelRef.Hole) {
						array[item] = deref(entry[item]);
					}
				}
