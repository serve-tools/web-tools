...
import { serve, type WorkerHandlers, type WorkerServer } from "@serve-tools/client-messaging";
...
export const activate = <Schema extends SchemaDefinition<Schema> = DB.Schema>(
	name: string,
	options?: DBOpenOptions<Schema>,
): SharedDBServer<Schema> => {
	const servers = new Set<WorkerServer<SharedDBProtocol<Schema>>>();
	...
	const handlers: WorkerHandlers<SharedDBProtocol<Schema>> = {
		requests: {
	...
	const connected = ({ ports }: ConnectEvent): void => {
		const port = ports[0];
		...
		const server = serve<SharedDBProtocol<Schema>>(port, handlers);

		servers.add(server);
		void server.closed.then(() => servers.delete(server));
	};
