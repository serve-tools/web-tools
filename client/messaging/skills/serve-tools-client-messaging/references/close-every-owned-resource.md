...
- If an endpoint's optional `start()` throws, `connect()` and `serve()` remove their protocol listeners and rethrow the original error without closing the transport.
  The server aborts operations accepted during startup, cancels its lease watch, and runs subscription cleanup even when returned after startup fails.
