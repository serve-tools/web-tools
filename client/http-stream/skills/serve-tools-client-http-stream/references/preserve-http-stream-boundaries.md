...
Keep `maximumMessageLength` aligned with the server's response limit when it differs from the client's 16 MiB default.
The client uses this positive limit for finite responses, stream frames, and resizable-buffer declarations before deserializing peer values.
