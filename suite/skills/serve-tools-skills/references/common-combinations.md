...
Use `@serve-tools/base-components` when the element should own a persistent layout and automatically suspend its signal bindings while disconnected.
In this guide version, Base is available only from the repository workspace and is not yet published to npm.
...
Base's `/template` subpath re-exports the same renderer; return an `html` result from `layout()` and the base supplies context and ownership automatically.
